...
export const PegaExtensionsDisplayAttachments = (props: UtilityListProps) => {
  ...
  const loadAttachments = useCallback(
    (response: Array<any> = []) => {
      ...
      setFiles(files => [...files, ...listOfFiles]);
      setAttachments(attachments => [...attachments, ...listOfAttachments]);
  ...
  const initialLoad = useCallback((caseID: string) => {
      ...
      (window as any).PCore.getDataApiUtils()
        ...
        .then((response: any) => {
          if (response.data.data !== null && Array.isArray(response.data.data)) {
            const caseIDs = response.data.data.map((item: any) => item.pzInsKey);
            caseIDs.forEach((id: string) => initialLoad(id));
          } else {
            setLoading(false);
          }
        })
