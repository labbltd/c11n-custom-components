...
export default {
  title: 'Widgets/Cases Attachments',
  argTypes: {
    dataPage: {
      table: {
        disable: true,
...
const setPCore = () => {
  (window as any).PCore = {
    ...
    getDataApiUtils: () => {
      return {
        getData: (dataView: string, payload?: any) => {
          if (dataView === 'D_CaseListRO') {
            return Promise.resolve({
              data: {
                data: [
                  { pzInsKey: 'OK5NFJ-COMPUTER-WORK A-11011', pyID: 'A-11011' },
                  { pzInsKey: 'OK5NFJ-COMPUTER-WORK B-22022', pyID: 'B-22022' },
                ],
              },
            });
          }
          const linkRefFrom = payload?.dataViewParameters?.[0]?.LinkRefFrom ?? '';
          ...
          return Promise.resolve({
            data: {
              data: linkRefFrom.includes('B-22022') ? allAttachments.slice(3) : allAttachments.slice(0, 3),
            },
          });
        },
      };
    ...
    getAttachmentUtils: () => {
      return {
        ...
        getCaseAttachments: (caseID: string) => {
          const allAttachments = [
            {
          ...
          return Promise.resolve(caseID.includes('B-22022') ? allAttachments.slice(3) : allAttachments.slice(0, 3));
        },
      };
    },
    getConstants: () => {
...
export const Default: Story = DisplayAttachmentsDemo({
  heading: 'Cases attachments',
  casesDataPage: 'D_CaseListRO',
  groupByCase: false,
  categories: '',
  useLightBox: false,
...
export const GroupedByCase: Story = DisplayAttachmentsDemo({
  heading: 'Cases attachments',
  casesDataPage: 'D_CaseListRO',
  groupByCase: true,
  categories: '',
